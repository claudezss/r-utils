...
pub async fn create_s3_client(region: &str, key: &str, secret: &str) -> s3::Client {
...
pub async fn get_object_content(
    ...
    let content = client
        ...
        .await
        .unwrap();
    let body = content.body.collect().await.unwrap().into_bytes();
...
pub async fn get_objects(
    ...
    for obj in objects.contents().unwrap_or_default().iter() {
        ...
        let dt: DateTime<Utc> = SystemTime::try_from(obj.last_modified.unwrap())
            .unwrap()
            .into();
        let mut content = "".to_string();
        if include_content {
            content = get_object_content(client, bucket_name, &key).await.unwrap();
        }
